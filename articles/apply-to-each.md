...
4. Envia uma notificação push e marca como lidos todos os e-mails que tenham **participar agora** no assunto, sejam estes do seu chefe ou marcados com importância alta.
...
1. Selecione **Os meus fluxos**> **Novo**> **Agendado a partir do zero**.
1. Forneça um nome para o seu fluxo em **Nome do fluxo** no ecrã **Criar um fluxo agendado**. 
1. Defina a agenda para ser executada a cada 15 minutos. 
1. Selecione **Criar**. 
   
    ![execuções da agenda](./media/apply-to-each/foreach-3.png) 

1. Selecione **+ Novo passo** e, em seguida, escreva **outlook** na caixa de pesquisa para localizar todos os conectores e as ações relacionadas com o Microsoft Outlook.
1. Selecione a ação **Obter e-mails (V3)** :
1. O cartão **Obter e-mails (V3)** é aberto. Configure o cartão **Obter e-mails (V3)** para selecionar os 10 principais e-mails não lidos na pasta Caixa de Entrada. Não inclua os anexos, pois estes não serão utilizados no fluxo:
...
1. Introduza o token **valor** no campo **Selecionar uma saída dos passos anteriores** no cartão **Aplicar a cada**. Esta ação obtém o corpo dos e-mails a utilizar na ação **Aplicar a cada um**:
...
1. Configure o cartão **Condição** para procurar no assunto da cada e-mail as palavras "participar agora":
   ...
   * Introduza o token **Assunto** no primeiro campo do cartão **Condição**.
...
1. Selecione **Adicionar uma ação**> **Condição** no ramo **Se sim**. Esta ação abre o cartão **Condição 2**. Configure esse cartão da seguinte forma:
   
   * Introduza o token **Importância** no primeiro campo.
   * Selecione **é igual a** na lista de operadores.
   * Introduza **elevado** no campo à direita.
     
     ![adicionar condição](./media/apply-to-each/foreach-importance-condition.png)
1. Selecione **Adicionar uma ação** na secção **Se sim**.     
   Esta ação abre o cartão **Escolher uma ação**, onde irá definir o que deve acontecer se a condição de pesquisa (o e-mail **participar agora** foi enviado com importância elevada) for verdadeira.
...
1. No cartão **Enviar-me uma notificação por telemóvel**, forneça os detalhes da notificação push que será enviada se o assunto de um e-mail contiver "participar agora" e a **Importância** for **elevada**.
   
    ![configurar notificação](./media/apply-to-each/foreach-11.png)

1. Volte ao cartão **Condição 2** e, no ramo **Se não**:
    
    * Selecione **Adicionar uma ação** e, em seguida, escreva **obter gestor** na caixa de pesquisa.
    * Selecione a ação **Obter o gestor (V2)** na lista de resultados da pesquisa.
    * Introduza o token **Para** na caixa **Utilizador (UPN)** do cartão **Obter o Gestor (V2)** .
...
1. Selecione **Condição** no cartão **Escolher uma ação**. Esta ação abre o cartão **Condição 3**. Configure o cartão para verificar se o endereço de e-mail do remetente da mensagem de e-mail (o token De) corresponde ao endereço de e-mail do seu chefe (o token E-mail):
    
    * Introduza o token **De** na primeira caixa.
    * Selecione **contém** na lista de operadores.
...
1. Adicione o token **ID da mensagem** ao cartão **Marcar como lido ou não lido (V2)** . Poderá ter de selecionar **Ver mais** para encontrar o token **ID da mensagem**. O token **ID da mensagem** é o ID da mensagem que será marcada como lida.
1. Selecione **Lida** na lista **Marcar como** no cartão **Marcar como lido ou não lido (V2)** .
...
1. Inicie sessão no Power Automate e selecione **Os meus fluxos**.
   
    Será apresentada uma lista dos seus fluxos. 
    
1. Selecione o fluxo que criou e, em seguida, selecione **Executar**.

   ![executar agora](./media/apply-to-each/run-flow.png)

1. Selecione **Página de Execuções de Fluxo** e, em seguida, selecione a execução de fluxo cujos resultados quer ver.
