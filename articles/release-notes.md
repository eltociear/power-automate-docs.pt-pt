...
3. Como funciona o licenciamento com o Office 365?
   ...
   * Se for um utilizador do Office 365, obtém acesso total através do Power Automate para o plano do Office 365. Para mais informações, consulte os [planos de preços do Power Automate](https://flow.microsoft.com/pricing/).
   * Se for um administrador, consulte as informações sobre o [licenciamento do Power Automate](organization-q-and-a.md), incluindo com o Office 365.
...
- **Teste o seu fluxo ao utilizar dados de exemplo** – utilize dados de exemplo de conectores para testar o fluxo à medida que o cria a partir do estruturador do Power Automate. Ao testar o seu fluxo com dados de exemplo, está a confirmar que o fluxo será executado conforme esperado quando for implementado para produção.
- **Cinco novos conectores** – adicionámos quatro novos conectores de gestão: Power Apps for App Makers, Power Platform for Admins, Power Apps for Admins, Power Automate for Admins e Microsoft School Data Sync.
...
- **Criar e executar fluxos a partir do Excel** – Com o novo botão de **Fluxo** (acedidos a partir do separador **Dados** do friso), pode criar e acionar automatizações de Power Automate nos dados da sua tabela no Excel. Automatize o processamento de dados ou a cópia/importação de dados.
...
- **Dois novos conectores** – Cloud Connect Studio e PoliteMail adicionados.
...
- **Trabalhar com fluxos mais amplos** – a aplicação móvel Power Automate agora suporta a vista horizontal e o estruturador Web tem uma barra de deslocamento horizontal.
...
- **Aprovações no Common Data Service** – as aprovações modernas baseiam-se na versão mais recente do Common Data Service. Isto significa que pode criar fluxos que leem o estado das aprovações que enviar ou receber com o conector Common Data Service.
...
- **Listas de salas** – o conector do Office 365 Outlook adicionou ações para obter dados de salas na sua organização.
...
- **Informações de perfil do Office 365** – adicionámos novas ações ao conector Utilizadores do Office 365 que funcionam com os perfis e as fotografias dos utilizadores.
...
O Power Automate Analytics está agora disponível em todas as regiões do Power Automate, ou seja, pode obter mais informações sobre o estado de funcionamento dos fluxos executados no seu ambiente.
...
- **Melhorias de ligações** – o Power Automate lembra-se das ligações mais utilizadas por si e mostra-lhe todos os conectores recentemente adicionados.
...
O Power Automate Launch Panel está agora disponível em todas as regiões. Este painel permite-lhe adicionar valores a um fluxo ao executá-lo na sua lista do SharePoint ou na biblioteca de documentos.
...
* **Conector do Azure AD** – Existe um novo conector para efetuar ações administrativas a partir do Power Automate, incluindo criar utilizadores ou adicioná-los a grupos.
* **Melhoramentos do Office 365 Outlook** – Os fluxos podem agora ser acionados por Caixas de Correio Partilhadas e enviar correio para uma Caixa de Correio Partilhada. Também pode definir ou ler respostas automáticas.
...
* **Fluxos de equipa na aplicação do Power Automate** – pode ver os fluxos com outros proprietários na aplicação Power Automate para iOS ou Android.
...
* **Dois novos serviços** – suporte extra para Notificações Power Apps e PivotalTracker.
...
* **Acionar os botões de fluxo** – agora, pode acionar botões de fluxo diretamente no Power Automate. Quando estiver a olhar para a lista de fluxos, basta selecionar o menu "…" e escolher o comando Executar agora.
...
* **DIsponibilidade Android** – a aplicação telefónica Power Automate está agora disponível para iOS e Android. A aplicação permite-lhe obter notificações, monitorizar a atividade e iniciar fluxos com o toque de um botão.
...
Power Automate agora disponível para todos – Inicialmente, abrimos a pré-visualização apenas para endereços de e-mail fornecidos pela sua empresa ou escola, como os utilizados com o Office 365 Empresas ou o Office 365 Enterprise. Hoje, estamos a anunciar que a pré-visualização está oficialmente disponível, de utilização gratuita, para todos os utilizadores, independentemente do e-mail que possam ter. [Leia mais informações e faça perguntas](https://flow.microsoft.com/blog/available-for-everyone/) sobre esta versão.
...
* **Apresentação da nova aplicação móvel para o Power Automate** – Hoje, temos o prazer de apresentar outra peça importante da nossa oferta: uma aplicação móvel agora disponível para transferência no iOS (brevemente, também no Android) que lhe permite gerir, controlar e explorar os seus fluxos de trabalho automatizados em qualquer altura e em qualquer lugar.  
