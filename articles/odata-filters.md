...
Embora estas instruções copiem dados de uma [lista](https://support.office.com/article/SharePoint-lists-I-An-introduction-f11cd5fe-bc87-4f9e-9bfe-bbd87a22a194) do Microsoft SharePoint (a origem) para uma tabela de [Base de Dados SQL do Azure](https://docs.microsoft.com/azure/sql-database/sql-database-technical-overview) (o destino), pode copiar dados entre vários dos mais de [150 serviços](https://flow.microsoft.com/connectors/) suportados pelo Power Automate.
...
* Acesso a uma origem de dados e a um destino de dados. Estas instruções não incluem passos para criar a origem e o destino.
* Acesso ao [Power Automate](https://flow.microsoft.com).
* Uma compreensão básica de como os dados são armazenados.
* Ter conhecimento das noções básicas da criação de fluxos. Pode rever como adicionar [ações, acionadores](multi-step-logic-flow.md#add-another-action) e [condições](add-condition.md). Os seguintes passos pressupõem que sabe como realizar estas ações.
...
2. Determine pelo menos uma coluna que identifique de forma exclusiva os itens na origem e no destino. No exemplo que se segue, utilizamos a coluna **Título**, mas pode utilizar as colunas que quiser.
3. Configure um acionador que monitorize alterações na origem.
...
6. Acione o seu fluxo e, em seguida, certifique-se de que os itens novos ou alterados estão a ser copiados da origem para o destino.
...
## <a name="monitor-the-source-for-changes"></a>Monitorizar as alterações na origem
1. Iniciar sessão no [Power Automate](https://flow.microsoft.com), selecione **Os meus fluxos** > **Criar do zero**.
...
## <a name="search-the-destination-for-the-new-or-changed-item"></a>Procurar itens novos ou alterados no destino
...
1. Selecione a caixa à esquerda.
    ...
    Será aberta a lista **Adicione conteúdo dinâmico a partir das aplicações e conectores utilizados neste fluxo**.
...
5. Selecione **Editar em modo avançado**.
...
> [!NOTE]
> O seu fluxo não é acionado quando um item é eliminado da origem. Se for um cenário importante, considere adicionar uma coluna separada que indica quando um item já não é necessário.
> 
> 
